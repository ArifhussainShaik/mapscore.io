  ...
  return (
    <section id="pricing" className="py-24 px-6 border-t border-slate-200/50">
      <div className="max-w-5xl mx-auto text-center">
        ...
        <p className="text-slate-600 mb-16">No subscriptions. No surprises. Your audit is yours to keep.</p>

        {/* Three cards, one screen, one decision (rule 12 + rule 22) */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-left max-w-5xl mx-auto items-stretch">
          {PACKAGES.map((pkg) => (
            <div
              key={pkg.id}
              className={`bg-white rounded-3xl p-8 shadow-sm border h-full flex flex-col ${
                pkg.featured ? "border-2 border-blue-600 shadow-xl relative md:-mt-4 md:mb-4" : "border-slate-100"
              }`}
            >
              {pkg.featured && (
              ...
              <div className="mb-8 flex items-baseline gap-1">
                ...
                <span className="text-sm text-slate-500">/ {pkg.unit}</span>
              </div>
